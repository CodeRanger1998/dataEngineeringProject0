...
def getDataFromSQLite(con):
    cur = con.cursor()
    data = cur.execute(""" 
        SELECT nature,COUNT(*) FROM incidents GROUP BY nature ORDER BY COUNT(*) DESC,name DESC;
""")
    return data
        ...
        for pageLine in pageLines:
            ...
            if incidentDate != None:
                ...
                if len(pageLine)>0:
                    ...
                    pageLine = re.split(incidentOri,pageLine)[0]
                    # address = re.search("([A-Z]?[a-z]+/*\s*)+",pageLine)
                    nature = re.search("(MVA)?(911)?(EMS)?\\s*[A-Z][a-z]+.*",pageLine)
